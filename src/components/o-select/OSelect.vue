<template>
	<div
		...
		<select :class="$style[`${className}-select`]" :name="id" :id="id" v-model="value">
			<option v-for="item in selectItems" :key="item.id" :value="item.id" @click="handleClick(item.id)">
				{{ item.text }}
			</option>
		</select>
	</div>
...
const selectItems = computed<ISelectItem[]>(() =>
	props.zeroId === 0 && props.showEmpty ? [{ id: 0, text: "Ничего не выбрано" }, ...props.items] : props.items
);
</script>
